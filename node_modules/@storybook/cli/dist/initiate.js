...
const installStorybook = (projectType, options) => {
  ...
  const REACT_NATIVE_DISCUSSION = 'https://github.com/storybookjs/react-native/blob/master/app/react-native/docs/manual-setup.md';
  ...
  const runGenerator = () => {
    switch (projectType) {
      ...
      case _project_types.ProjectType.UPDATE_PACKAGE_ORGANIZATIONS:
        return (0, _UPDATE_PACKAGE_ORGANIZATIONS.default)(packageManager, options.parser, npmOptions).then(() => null) // commmandLog doesn't like to see output
      ...
      case _project_types.ProjectType.REACT_NATIVE:
        {
          return (options.yes ? Promise.resolve({
            ...
            logger.log(_chalk.default.cyan(REACT_NATIVE_DISCUSSION));
            logger.log();
          });
        }
...
const projectTypeInquirer = async options => {
  const manualAnswer = options.yes ? true : await _inquirer.default.prompt([{
    type: 'confirm',
    name: 'manual',
  ...
  if (manualAnswer !== true && manualAnswer.manual) {
    const frameworkAnswer = await _inquirer.default.prompt([{
      type: 'list',
      name: 'manualFramework',
      message: 'Please choose a project type from the following list:',
      choices: _project_types.installableProjectTypes.map(type => type.toUpperCase())
    }]);
